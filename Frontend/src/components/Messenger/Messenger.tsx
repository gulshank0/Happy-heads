...
export default function Messenger() {
  ...
  const [connectionStatus, setConnectionStatus] = useState<
    "connected" | "disconnected" | "connecting" | "failed" | "fallback"
  >("disconnected");
  ...
  useEffect(() => {
    ...
    const handleError = (data: {
      ...
      if (data.type === "error" && data.data) {
        const errorMessage =
          data.data.message || data.data.error || "Unknown WebSocket error";
    ...
    unsubscribeFunctions.push(
      websocketService.on("reconnect-failed", handleReconnectFailed),
    );
    unsubscribeFunctions.push(
    ...
    unsubscribeFunctions.push(
      websocketService.on("messages-read", handleMessagesRead),
    );
    unsubscribeFunctions.push(
      websocketService.on("user-status-changed", handleUserStatusChanged),
    );
  ...
  // Add effect to automatically mark messages as read when viewing them
  useEffect(() => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      setMessages((prev) =>
        prev.filter((msg) => msg.id !== `temp_${Date.now()}_${Math.random()}`),
      );
    } finally {
  ...
  return (
    <div className="h-screen bg-gray-900 flex">
      ...
      <div
        className={`flex w-full ${connectionStatus !== "connected" ? "pt-10" : ""}`}
      >
              ...
              <div className="p-4 border-b border-gray-700 flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  <div className="relative">
                    <img
                      src={
                        currentConversation.user.avatar || "/default-avatar.png"
                      }
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-400">
                      {currentConversation.user.isOnline
                        ? "Online"
                        : `Last seen ${currentConversation.user.lastSeen}`}
                    </p>
                  </div>
                </div>
