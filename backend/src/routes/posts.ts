  ...
  try {
    ...
    const post = await prisma.post.create({
      data: {
        title: title || '',
        content: content || '',
        image: imageUrl, // Add this line to save image URL
        authorId: userId,
        published: true
      },
      include: {
...
router.get('/feed', requireAuth, async (req, res) => {
  try {
    const userId = req.user?.id;
    console.log('📡 Feed endpoint called by user:', userId);
    ...
    const formattedPosts = posts.map(post => ({
      ...
      userName: post.author.name || 'Anonymous User',
      userAvatar: post.author.avatar || `https://api.dicebear.com/8.x/lorelei/svg?seed=${post.author.name}`,
      userAge: post.author.age || 20,
      userCollege: post.author.college || 'College',
      userLocation: post.author.location || 'Location',
      content: post.content || '',
      image: post.image, // Include the image field
      timestamp: formatTimestamp(post.createdAt),
      likes: post._count.likes,
      comments: post._count.comments,
      isLiked: post.likes.some(like => like.userId === userId),
      isBookmarked: false, // You can implement this later
      tags: [], // Extract from content if needed
      privacy: 'public' as const
    }));

    console.log('✅ Sending formatted posts:', formattedPosts.length);
...
router.delete('/:postId', requireAuth, async (req, res) => {
  try {
    ...
    // Delete the post (this will cascade delete comments and likes due to foreign key constraints)
