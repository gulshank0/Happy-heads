...
export class MatchingController {
  ...
  async getDiscoveryUsers(req: Request, res: Response) {
    try {
      const userId = (req.user as any)?.id;
      if (!userId) {
        return res.status(401).json({ error: 'Not authenticated' });
      ...
      const limit = parseInt(req.query.limit as string) || 10;
      const users = await matchingService.getUsersForDiscovery(userId, limit);
      ...
      res.status(500).json({
        success: false,
        error: error instanceof Error ? error.message : 'Failed to get discovery users'
      });
    }
  }

  // Handle swipe action (like or pass)
  async handleSwipe(req: Request, res: Response) {
    try {
      const userId = (req.user as any)?.id;
      ...
      if (!targetUserId || !action || !['like', 'pass'].includes(action)) {
        return res.status(400).json({ error: 'Target user ID and valid action (like/pass) are required' });
      }

      const result = await matchingService.recordSwipeAction(userId, targetUserId, action);
      ...
      res.status(500).json({
        success: false,
        error: 'Failed to handle swipe action'
      });
    }
  }
}